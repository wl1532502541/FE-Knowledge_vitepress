...
![](https://cdn.nlark.com/yuque/0/2021/webp/12821255/1626357897764-38091db6-91bb-4529-9680-180cc0281583.webp#align=left&display=inline&height=665&margin=%5Bobject%20Object%5D&originHeight=665&originWidth=1280&size=0&status=done&style=none&width=1280)
#### 2.4 题目四
```
const promise1 = new Promise((resolve, reject) => {
  setTimeout(() => {
...
1. `.then`和`.catch`都会返回一个新的`Promise`。(上面的👆1.4证明了)
1. `catch`不管被连接到哪里，都能捕获上层未捕捉过的错误。(见3.2)
1. 在`Promise`中，返回任意一个非 `promise` 的值都会被包裹成 `promise` 对象，例如`return 2`会被包装为`return Promise.resolve(2)`。
1. `Promise` 的 `.then` 或者 `.catch` 可以被调用多次, 但如果`Promise`内部的状态一经改变，并且有了一个值，那么后续每次调用`.then`或者`.catch`的时候都会直接拿到该值。(见3.5)
1. `.then` 或者 `.catch` 中 `return` 一个 `error` 对象并不会抛出错误，所以不会被后续的 `.catch` 捕获。(见3.6)
1. `.then` 或 `.catch` 返回的值不能是 promise 本身，否则会造成死循环。(见3.7)
1. `.then` 或者 `.catch` 的参数期望是函数，传入非函数则会发生值透传。(见3.8)
1. `.then`方法是能接收两个参数的，第一个是处理成功的函数，第二个是处理失败的函数，再某些时候你可以认为`catch`是`.then`第二个参数的简便写法。(见3.9)
...
1. `.finally()`方法的回调函数不接受任何的参数，也就是说你在`.finally()`函数中是没法知道`Promise`最终的状态是`resolved`还是`rejected`的
...
**拓展题**
这道拓展题来自于“万物皆可爱的[LINGLONG](https://juejin.im/user/2436173498955032) ”小姐姐，炒鸡棒 😁👍。
...
![](https://cdn.nlark.com/yuque/0/2021/webp/12821255/1626357897791-69df6288-2c0c-422b-b367-39ab05c6746d.webp#align=left&display=inline&height=804&margin=%5Bobject%20Object%5D&originHeight=804&originWidth=1280&size=0&status=done&style=none&width=1280)
为了方便你查看，我截了个图，不过代码在后面也有
(说真的，要我看这一大长串代码我也不愿意...)
![](https://cdn.nlark.com/yuque/0/2021/webp/12821255/1626357897801-d7ac1182-8591-4407-8697-0327bc34953b.webp#align=left&display=inline&height=1280&margin=%5Bobject%20Object%5D&originHeight=1280&originWidth=864&size=0&status=done&style=none&width=864)
代码：
```
function limitLoad(urls, handler, limit) {
...
作者：LinDaiDai_霖呆呆
链接：[https://juejin.cn/post/6844904077537574919](https://juejin.cn/post/6844904077537574919)
来源：掘金
